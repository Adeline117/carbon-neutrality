...
Toucan 后来还做了第二个池子叫 NCT（Nature Carbon Tonne），跟 BCT 的区别是 NCT 只收自然类信用（森林保护、造林这些），相当于加了一个简单的类型门槛。NCT 在我后面的分析里会反复出现，因为它是 BCT 的天然对照组：同样的区块链、同样的协议，唯一的区别就是有没有质量筛选。
...
这里有个统计上需要小心的地方。BCT 的存款是按钱包聚类的，有些大钱包一口气存了好多笔，直接做朴素二项检验会夸大显著性。我用了三种方式处理这个聚类问题：钱包级置换检验（随机打乱钱包标签 10,000 次）、HHI 调整（HHI 是衡量集中度的指标，用它算出有效独立样本只有 83.5 个而不是 1,187 个）、DEFF 调整（考虑组内相关性后有效样本 270 个）。三种方法下全部高度显著，最弱的 HHI 调整下都是 p = 2.9 × 10⁻¹⁵。
...
分析完存款端，接下来看赎回端。规则是花 1 个 BCT 代币可以从池子里取出 1 吨碳信用。碳信用在区块链之外还有一个传统的交易市场（链下市场），企业通过经纪商或交易平台按项目类型和质量买卖碳信用，价格差异很大。如果 BCT 市场价 $2 但你取出的信用在链下市场值 $10，差价就是利润。
...
这个"双边"的结构比较少见。经济学里经典的逆向选择模型（Akerlof 的"柠檬市场"，后面理论部分会详细说）假设买方和卖方是同一个市场里的同一群人，信息不对称导致差商品挤掉好商品。但 BCT 的情况是两个几乎完全独立的群体分别操作了池子的两端，存款端和赎回端的激励结构完全不同。
...
后来我换了一个思路：与其单独测"NCT 有没有下降"，不如测"BCT 是不是比 NCT 下降得更快"。把两个池子 1,895 笔存款合成一个面板数据做 DiD（差分中的差分，一种比较两组在某个冲击事件前后差异变化的方法），以 2022 年 5 月 Terra 崩盘（Terra 是一个算法稳定币项目，崩盘导致了整个 DeFi 市场的流动性危机，BCT 价格也跟着暴跌）作为外生冲击。
...
为了验证这个不只是碳市场的个例，我还分析了以太坊上另一个完全不同的 DeFi 市场。Curve Finance 有一个 stETH/ETH 池，把 stETH（一种以太坊质押衍生品，理论上应该跟 ETH 等值，但实际上有交易对手风险）和 ETH 按近似 1:1 的价格放在一起交易。结构上跟 BCT 很像：两种质量不同的资产被当作等价的东西混在一起。

2022 年 5 月 Terra 崩盘的时候完全相同的模式出现了：stETH 的风险暴露了，聪明的人赶紧把池子里的 ETH（高质量资产）取走，留下 stETH（风险资产）。3 周内 380,869 ETH 等值的组成转移（约 $7.62 亿），是正常波动的 40 倍。跟 BCT 一样的故事：统一定价 + 质量差异 = 好资产被选择性取走。
...
设计了 7 个维度的评分（0-100 分）：减排类型层级 25%、额外性 20%、MRV（监测、报告、核查）20%、持久性 17.5%、年份 10%、注册机构质量 7.5%，外加一个不计分的共同效益门槛（有人权或环境风险就一票否决）。权重参考了 Oxford Offsetting Principles 和 ICVCM CCP 框架（碳信用领域最权威的两个质量标准）。这些权重是初始值，计划后续通过专家共识来校准（详见局限性部分）。

综合分后我定义了一个指标叫 PQD（Pool Quality Deficit，池质量赤字），就是 1 减去池子里所有信用的吨位加权平均分再除以 100。PQD 越高表示池子质量越差，0 表示完美，1 表示全是垃圾。BCT 的 PQD 是 0.679，意思是池子的平均质量只有满分的 32.1%。

自己设计的评分总要回答一个问题：你这个评分靠谱吗？不是在自说自话吗？

所以做了三种外部验证：

1. **CCP 标签对比**（n=318）：ICVCM 审批通过的信用平均得分比没通过的高 1.99 个等级，Cohen's d = 1.87（d 是衡量两组差异大小的标准化指标，0.8 以上就算大效应，1.87 是非常大的分离）。然后我还做了循环论证检验：去掉权重最大的维度（减排类型 25%），CCP/非CCP 的差距不但没缩小反而保留了 101.4%。说明分离是由额外性、持久性、MRV 共同驱动的，不是某一个维度造成的假象。

2. **BeZero 商业评级对比**（n=27，跨 12 种方法学类型）：Spearman ρ = +0.901（ρ 衡量排名一致性，1.0 是完美一致，0.7 以上算强相关），100% 在 ±1 级内吻合。比三家商业评级公司之间的互评一致性还高（它们之间平均 ρ 才 0.009，基本随机）。

3. **LLM 评审员面板**（3 个模型 × 29 个信用）：Fleiss' κ = 0.600（κ 衡量多个评审者之间的一致性，0.6 算中等偏上，说明不同 AI 模型独立打分后得出了相似的结论）。权重最大的两个维度（减排类型 κ=0.585，持久性 κ=0.684）一致性最高，框架里负荷最大的维度恰好是最可复现的。
...
| 16 | KlimaDAO 金库排除 | KlimaDAO 是另一个大量持有 BCT 的 DeFi 协议，有人可能会说 BCT 的组成偏差是 KlimaDAO 造成的。检查去掉 KlimaDAO 相关钱包后偏差是否还在 | KlimaDAO 钱包交叉比对 | 不能解释，偏差在 KlimaDAO 之外也成立 |
...
| # | 实验 | 做了什么 | 数据规模 | 关键结果 |
|---|------|---------|---------|---------|
| 28 | 多重检验校正 | 做了 10 项假设检验，用 FDR（False Discovery Rate，控制假阳性率的方法）校正，避免"做的检验越多越容易蒙到显著"的问题 | 10 项检验 | 6 项通过 α=0.05 |
...
| 40 | Lemons Index 系统扫描 | Lemons Index（柠檬指数）是我定义的逆向选择严重程度指标，0 表示没有逆向选择，1 表示全是"柠檬"。把 VCM 按类型/注册库/年份/CCP 状态切成约 30 个合成池段，计算每段的指数 | 30 个池段 | BCT LI=0.724（严重），CHAR LI=0.221（轻微），随机基准 LI=0.51 |
...
2. **质量门槛可行且有效**。反事实模拟显示 BBB 门槛把池质量赤字（PQD）从 0.679 降到 0.405，改善 40%。Toucan 后来出的 CHAR 池（只收高质量的生物炭项目，PQD=0.221）在实践中验证了这条路。
...
- 上面六个发现对应的主论文。完整的 BCT 链上取证 + 质量评分框架验证 + 价格质量反馈 + 赎回取证 + 跨池比较 + DiD + 跨域验证。
- ~13,750 词 + 8 图 6 表 44 引用
- 对应实验：#1-32, #38-43

**论文 B：链上质量评级技术标准**
- 标题：ERC-CCQR: The Missing Composability Primitive for Real-World Asset Quality
- 这篇是技术标准论文。设计了一个智能合约接口，让 DeFi 池子可以在碳信用存入或取出的时候自动检查质量等级。核心是一个叫 `meetsGrade()` 的函数，任何协议都可以调用它来判断"这个信用够不够好"，不需要额外的 gas 费用。
- 包含完整的智能合约代码（Solidity）和自动化测试
- 验证了在 BCT 上应用质量门槛的效果，还跨域测试了生物多样性信用和可再生能源证书
- 完整版 10,382 词 + 精简版 3,355 词
- 对应实验：#4-13, #38-41, #45

### 已合并（不再是独立论文）

早期有两篇单独的论文，后来并进了论文 A：

- **框架验证论文**：CCP 标定、商业评级对比、LLM 面板、质量图谱这些内容，原来是单独一篇。后来发现如果框架验证单独发，主论文的审稿人会说"你的评分没验证过"，合在一起才完整。
- **监管映射论文**：六个监管框架的对比、福利损失估计。跟框架验证论文有 85% 内容重叠，单独投有自我抄袭风险。监管映射移到了主论文的补充材料里。

### 扩展方向（想法和规划，还没有成形）

下面这几个是不同阶段的探索方向，有的只是个想法，有的有初步骨架但需要合作者才能往前推。具体投什么会议期刊还需要跟教授讨论。

**方向 1：监管收敛悖论（草稿）**
- 为什么六个独立监管框架（CCP, Article 6.4, CORSIA, EU CRCF, VCMI, 新加坡 ICC）都收敛到了二元门槛（通过/不通过），而二元门槛在结构上不足以解决逆向选择。
- CCP 通过的池子 Lemons Index（逆向选择指数，后面实验 #40 会解释）仍有 0.419，只改善了 37%。
- 短文（≤2,000 词），可独立投稿
- 状态：草稿

**方向 2：卫星验证（需要合作者）**
- 用卫星数据独立验证 BCT 里碳信用的实际减排效果。初步结果显示 18 个印度可再生能源项目高估电网替代 20-29%；12 个 REDD+ 项目高估避免毁林 5.6 倍。
- 需要遥感方向的合作者
- 状态：MVP 骨架，等合作者
- 对应实验：#35-37

**方向 3：LLM 评分基准（早期草稿）**
- 把 LLM 评分面板做成一个可复现的 benchmark。168 个信用的 MVP 已完成，计划扩展到 500-2000 个信用，多个 LLM 供应商。
- 状态：草稿 v0.1，部分数字是 placeholder
- 对应实验：#11-12, #44

**方向 4：机制设计（想法阶段）**
- 设计了一个保证金机制，存款者按声称的质量等级交押金，事后验证质量，如果等级不符就扣押金。理论上能强制分离均衡。
- 需要博弈论方向的合作者完成形式化证明
- 状态：早期草稿
- 对应实验：#42
...
论文 A 是核心，论文 B 提供技术工具。四个扩展方向围绕核心展开，但目前都还在想法阶段，能不能做成论文要看合作者和资源。

47 个实验里绝大部分（#1-34, #38-43, #47）服务于两篇主论文。卫星相关的实验（#35-37）是为方向 2 准备的，机制设计模拟（#42）是方向 4 的，LLM 相关的（#11-12, #44）可以复用到方向 3。
