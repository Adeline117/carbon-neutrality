...
这个项目不只是一篇论文。我们规划了一个多论文的研究组合，围绕"代币化市场中的质量问题"这个主题从不同角度切入。最初规划了 4 篇，后来合并成 2 篇主投稿 + 4 篇不同阶段的扩展方向。
...
### 已合并的论文（内容吸收进 Paper A）

**原 Paper C：ERL（Environmental Research Letters）**
- 原来是单独的质量评分框架验证论文（CCP 标定、商业评级对比、LLM 面板、质量图谱、权重稳健性）
- 2026-04-27 决定合并进 Nat Comms 的 Methods + Results 2.5-2.6
- 因为框架验证如果单独发，Nat Comms 的审稿人会说"你的评分没验证过"；合在一起才完整

**原 Paper D：Nature Sustainability**
- 原来是监管映射 + 经济影响论文（六个监管框架对比、福利损失估计、政策建议）
- 跟 ERL 有 85% 内容重叠，单独投有自我抄袭风险
- 监管映射移到 Nat Comms 的 Supplementary Note 1，经济影响保留在 Discussion

### 扩展方向（不同阶段的草稿）

**Paper E：Nature Sustainability Perspective**
...
**Paper G：NeurIPS 2027 Datasets & Benchmarks**
...
```
              Nature Comms (Paper A)
             /        |          \
   质量评分框架    BCT 取证     跨域验证
        |             |            |
   WWW (Paper B)   Nature 卫星(F)  NeurIPS LLM(G)
   [技术标准]      [物理验证]      [AI 基准]
        |
   QRX (Paper H)
   [机制设计]
...
Paper A 是核心，其他论文围绕它展开。Paper B 提供技术工具，Paper F 用卫星数据独立验证 Paper A 的质量判断，Paper G 把评分方法做成 benchmark，Paper H 提出解决方案，Paper E 从监管角度补充政策讨论。
...
|---------|-----------|
| #1-3（数据采集） | Paper A |
| #4-13（评分框架） | Paper A + Paper B + Paper G |
| #14-20（因果识别） | Paper A |
| #21-27（价格/赎回取证） | Paper A |
| #28-32（统计稳健性） | Paper A |
| #33-34（Curve/NFTX 跨域） | Paper A |
| #35-37（卫星验证） | Paper F |
| #38-41（门槛/图谱/Lemons） | Paper A + Paper B + Paper E |
| #42（QRX 债券） | Paper H |
| #43（福利损失） | Paper A |
| #44（专家 BWS） | Paper G |
| #45（可扩展性） | Paper B |
| #46（RCT 管线） | 未来实地实验 |
| #47（Living paper） | Paper A 维护工具 |
...
| 东西 | 情况 |
|------|------|
| Paper A 主论文 | ~13,750 词，完稿，8 图 6 表 44 引用 |
| Paper B 技术标准 | 完整版 10,382 词 + 精简版 3,355 词，含 Solidity + Foundry |
| Paper E 观点文章 | 草稿，~2,000 词 |
| Paper F 卫星验证 | MVP 骨架，等合作者 |
| Paper G LLM 基准 | 草稿 v0.1，168 信用 MVP 完成 |
| Paper H 机制设计 | 早期草稿，等博弈论合作者 |
