...
Seven disqualifier flags cap the maximum achievable grade regardless of composite:
...
**Caveat**: n=6 REDD+-only with wide confidence intervals; the cross-type study below resolves this.

### 7.4b Cross-type rank correlation (v0.6)

To test whether the near-zero REDD+ inter-agency agreement is project-type-specific, we extended the study to 9 cross-type projects spanning CDR, biochar, cookstoves, IFM, methane abatement, landfill gas, and renewable energy, using publicly available BeZero, Calyx, and Sylvera ratings. Full analysis in `data/rank-correlation/cross_type_notes.md`.

**Overall cross-type Spearman vs BeZero: +0.906** (n=9), with 5 of 9 exact grade matches — dramatically stronger than the REDD+-only result (+0.664). This confirms that the near-zero inter-agency agreement documented in §7.4a is a property of REDD+ specifically (where baselines are contested), not of carbon credit rating in general.

**One systematic divergence identified**: cookstoves. Our framework caps avoidance-based projects at BBB due to low removal_type (38) and permanence (8) scores per the Oxford Principles hierarchy, while BeZero and Calyx rate leading cookstove projects at A-AA. This reflects a deliberate design choice: we weight removal above avoidance. The divergence is documented and defensible but should be discussed with experts (questionnaire Section 3).
...
## 8.5 Normative Assumptions

This framework embeds several value judgments that are not derivable from data. We name them explicitly so that users, reviewers, and expert consultees can evaluate them as choices rather than facts.

**Assumption 1 — Removal is more valuable than avoidance (Oxford Principles hierarchy).** The `removal_type` dimension assigns higher scores to engineered carbon dioxide removal than to avoided emissions. This reflects the Oxford Principles position that removal with durable storage is necessary for net-zero, while avoidance reduces the flow of emissions without removing existing atmospheric stock. The consequence is that cookstove projects (avoidance-based, impermanent) are structurally capped below A regardless of their community value. An alternative framework that weighs development impact equally with climate integrity would assign cookstoves higher grades. Our choice is defensible for climate-purist institutional buyers but may underweight the social value proposition that makes cookstove credits the dominant retirement category in the voluntary market.

**Assumption 2 — Co-benefit narratives should not inflate quality grades (safeguards-gate).** We dropped co-benefits from a 10% weighted dimension to a 0%-weight informational field with a `communityHarm` disqualifier gate. This decision is now supported by six independent sources converging on the same conclusion: (1) Berg et al. (2025): buyers pay 2x for co-benefit narratives regardless of integrity; (2) BeZero (2024): SDG claims command 31-61% premiums independent of integrity rating; (3) Calyx Global (2025): explicitly separates GHG integrity from SDG impact as two independent rating scales, never blended; (4) Sylvera (2025): excludes co-benefits from integrity scores to "prevent masking poor GHG performance"; (5) Lou et al. (npj Climate Action 2023): companies buy co-benefit-rich credits for reputational value, not climate integrity; (6) Oeko-Institut/Calyx (2024): cookstove projects — the archetype of co-benefit richness — have 9-10x overcrediting on average. All three major commercial rating agencies (BeZero, Calyx, Sylvera) independently arrived at the same architectural choice: co-benefits are scored separately or excluded from integrity composites. The alternative — keeping co-benefits as a scored dimension — remains configurable (change one weight in `index.json`) but is not supported by the available evidence.

**Assumption 3 — Additionality is assessable at the project level.** We score additionality 0-100 per credit based on financial analysis and common-practice tests. But the concept itself is a counterfactual — "would this project have happened without carbon finance?" — and counterfactuals are inherently unobservable. Coglianese & Giles (Science 2025) showed that third-party auditors systematically approve non-additional projects because the conflict-of-interest structure incentivizes approval. Our dimension may be scoring assessor opinion, not objective reality. We mitigate this with the IRR study (additionality κ = 0.243, "fair" agreement — the third-weakest dimension), which at least quantifies how much assessors disagree.

**Assumption 4 — Quality rating makes carbon credits more useful, not more harmful.** By building a quality rating framework, we implicitly endorse the premise that carbon credits can be a net positive if properly rated. Cheong (Anthropocene Science 2025) argues the opposite: that carbon markets delay structural decarbonization by allowing emitters to substitute cheap offsets for hard emission cuts. Zeng et al. (Nature Reviews Biodiversity 2026) found offset projects increase habitat disturbance by 3.7%. Our framework does not resolve this debate. It positions itself as: "if carbon credits exist (and they do, at $2B/year), honest quality differentiation is better than quality indistinguishability, even if the best outcome would be no credits at all."

**Configurability.** The linear composite and machine-readable rubric are designed so that users who disagree with these assumptions can modify them. A user who values co-benefits can restore the 10% weight by changing one line in `data/scoring-rubrics/index.json`. A user who rejects the Oxford hierarchy can flatten the removal_type bands. The framework is a mechanism, not a mandate.
...
51. BeZero Carbon. (2024). Buyers Pay More for Carbon Credits With SDG Claims. bezerocarbon.com.
52. Lou, J. et al. (2023). Corporate motivations and co-benefit valuation in private climate finance investments through voluntary carbon markets. *npj Climate Action*, 2, 63.
53. Liu, Y. (2025). Carbon Inequality: Price Discovery and Market Dynamics in the Voluntary Carbon Market. *WFE Research / SSRN 5861427*.
54. Oeko-Institut & Calyx Global. (2024). Cooking up Quality: Carbon credits from efficient cookstove projects face integrity issues worth fixing.
55. Sylvera. (2025). The Gap Between Carbon Credit Quality and Carbon Credit Value. sylvera.com.
56. Calyx Global. (2025). Calyx Global Ratings Explained: Separate GHG and SDG rating scales. calyxglobal.com.
