...
We collected all deposit transactions to the Toucan Protocol BCT pool contract on the Polygon blockchain using a Polygon RPC endpoint. The BCT pool contract accepts Toucan-bridged TCO2 tokens, each representing a specific Verra VCS project and vintage year. We identified {{composition.n_deposits}} deposit transactions spanning the period from BCT's launch in October 2021 to the effective cessation of new deposits in late 2022. These {{composition.n_deposits}} deposits mapped to {{composition.n_projects}} unique VCS project identifiers (345 unique TCO2 token addresses, reflecting multiple vintages per project) and approximately 22 million tonnes of tokenized carbon credits.
...
**Difference-in-differences.** To test whether BCT's quality trajectory diverged from NCT's --- rather than testing each pool's temporal trend in isolation (which is underpowered for NCT with $n$ = 35 unique tokens) --- we constructed a deposit-level DiD panel pooling all 1,895 events (1,187 BCT + 708 NCT). The estimating equation was:
...
where $\text{BCT}_i$ is a pool indicator, $\text{PostTerra}_i$ indicates deposits after the Terra/LUNA collapse (Polygon block 28,400,000, approximately 9 May 2022), and $\beta_3$ captures the BCT-specific quality change relative to NCT. Standard errors were cluster-robust at the TCO2 token level (349 clusters) using a CR1 sandwich estimator with $(G/(G-1)) \times ((n-1)/(n-k))$ small-sample correction. Inference on $\beta_3$ used the $t$-distribution with $G-1$ degrees of freedom. Three robustness checks supplemented the parametric DiD: (i) a permutation test shuffling pool labels across 10,000 iterations; (ii) a Bayesian bootstrap (10,000 Dirichlet-weighted draws) to obtain the posterior distribution of $\beta_3$; and (iii) a continuous-time specification replacing $\text{PostTerra}$ with standardised block number. We also estimated a tonnage-weighted variant using block-bootstrap standard errors (2,000 iterations resampling at the token level).
...
The type-level redemption rate differentials (renewable {{redemption.renewable_redeemed_pct}}% vs. non-renewable 91--100%) are self-evidently large and do not require a formal significance test for the primary finding. We note that a chi-squared test on event counts ($\chi^2$ = 4.0e6, $p$ $\approx$ 0, df = 1) confirms the differential is non-zero, but the test statistic is inflated by the large sample size (35,432 events) and the magnitude of the differential (3.6% vs. 91--100%) is the substantively meaningful quantity.
...
Tonnage-weighted mean quality of redeemed credits ({{redemption.mean_quality_redeemed_weighted}}) exceeded that of deposited credits ({{redemption.mean_quality_deposited_weighted}}), with a difference of 7 quality points. This pattern --- higher-quality credits being selectively withdrawn --- is consistent with the Gresham prediction that undervalued assets exit the pool when they can command higher prices elsewhere.
