...
## Supplementary Discussion: the cheapest-to-deliver parallel

The incentive structure this paper documents has a mature analogue in the US agency mortgage-backed-securities (MBS) market, where most trading occurs through the to-be-announced (TBA) contract. A TBA trade fixes only six coarse parameters, and any security meeting them is equally deliverable at the contract price, even though securities within a cohort differ materially in value (Vickery and Wright 2013 [ref 25]; Fusari, Li, Liu and Song 2022 [ref 26]). The parallel runs point for point. (i) One price clears observably different assets: the fungibility rule severs price from quality in both markets. (ii) The uniform price converges toward the worst eligible delivery, because the counterparty holds the selection option (ref 25); BCT's price tracking its degrading composition is the same repricing run to completion in a pool with no quality floor. (iii) Low-quality assets are strategically routed into the uniform-price venue: An, Li and Song (2025) [ref 27] show MBS issuers pool low-quality loans for TBA delivery, the venue that cannot discount them; BCT's 1.87x renewable over-selection is the carbon-market measurement of the same behaviour. (iv) High-quality assets migrate to a venue that prices differentiation: the specified-pool market in MBS, off-chain sales and the NCT pool in carbon (39.2% of extracted tonnage was re-deposited into NCT). (v) A two-tier structure is the equilibrium: TBA/specified-pool in MBS; unscreened/screened pools in carbon. Huh and Kim (2023) [ref 28] state directly that the TBA market induces adverse selection through cheapest-to-deliver pricing.

Two boundaries of the analogy are deliberate. First, observability: TBA quality is only partially observable ex ante, whereas every BCT credit's metadata was public in real time; our case therefore shows the mechanism requires no information friction at all, only the pricing rule. Second, margins: TBA is single-margin (the seller's delivery option at entry; the buyer cannot return the security), whereas BCT exhibits selection at both entry and exit. The exit margin we document is absent from the TBA literature because the TBA structure forecloses it; it is this paper's addition to the cheapest-to-deliver record, not a restatement of it.

## Full methods detail (relocated from the main text)

The main text carries a compact Methods summary (ERL Letter format); the full specifications previously in the main text are preserved here verbatim (LaTeX notation retained).
...
\textbf{Price data.} Daily BCT prices in US-dollar terms were obtained from the DeFi Llama API (Polygon DEX pricing for the BCT token; 1,493 daily observations, October 2021 to July 2026, re-fetched and committed via \texttt{fetch\_bct\_prices.py}).
...
**Token-level redemption differential.** The FDR family's member (10) is a token-level Mann-Whitney comparing redemption rates of non-renewable versus renewable tokens (n = 45 vs 116, U = 3,940, one-sided p = 1.95e-08; `redemption_differential_token_level.json`); the tonne-level chi-squared previously computed for this contrast is denominator-inflated by million-tonne cells and is not used for inference.
