...
Deposit quality declined over BCT's operating life (Spearman $\rho$ = $-$0.24, $n$ = 1,187). This was entirely a vintage effect: later deposits drew on progressively older vintages, the years in which the credited reductions occurred, rather than on worse credits from the same vintage (Supplementary Table 2). The May 2022 cryptocurrency crash offers a useful timing check on the proposed feedback loop. After the shock, deposit quality fell 3.4 points (Cohen's $d$ = 0.49, a moderate effect), volume fell 98\% (Fig.~2b), and the quality-decline slope tripled. The pattern is consistent with price falling first and deposit quality then worsening; it is not a clean causal experiment (Supplementary Methods).
...
Credit type, rather than our quality framework, captures the main pattern of selective redemption. A rule using type alone predicts redemption better than one using quality grade. Within the 116 renewable tokens, neither vintage nor grade predicts redemption (Supplementary Table 3). The paper's core findings---composition, base-rate over-selection, redemption-side extraction, and the within-token comparison---therefore rest on transaction data and Verra credit-type labels, not on our scoring framework (Supplementary Table 6).

The result is a large stranded inventory: 9.3 of 9.6 million deposited B-grade tonnes remain unredeemed. The scored analysis covers 15.2 million tonnes, making BCT one of the largest pools of stranded carbon credits on record.

\section*{Discussion}
In BCT, public credit information and transaction records did not prevent quality deterioration. This does not mean that all participants shared a common measure of quality: the registry and ledger made provenance and project attributes available, while our quality assessment combines those attributes with published evidence. The case instead shows why disclosure alone can fail when a contract treats heterogeneous credits as interchangeable.
...
The proposed mechanism is simple. If assets of different quality enter a pool at one price, while some units retain greater value outside it, a holder may prefer to withdraw those units. Less sought-after units can then accumulate. Disclosure can reveal this price gap without removing it. Traditional finance documents related incentives as cheapest-to-deliver pricing in the agency mortgage-backed-securities market\textsuperscript{25,26,27,28}. BCT adds an observable credit-level entry and exit record, allowing this mechanism to be studied directly rather than inferred from aggregate prices.
