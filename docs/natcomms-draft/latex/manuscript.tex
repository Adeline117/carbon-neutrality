...
A comparison of identical credits across two pools with different quality screens sharpens the finding. The same credits were 100\% redeemed from BCT but only 28.5\% from a quality-gated pool, a contrast consistent with pool design, rather than credit type, shaping asset fate. Unlike prior work that quantifies overcrediting at the project level\textsuperscript{4,5,22,23}, we trace how overcredited assets are selectively extracted once they enter a pooled market, even when quality differences are publicly observable.
...
An independent physical cross-check points the same way: Sentinel-5P methane retrievals over 9 BCT waste/methane projects found 5 of 9 with site concentrations not significantly below background, bounded by TROPOMI's $\sim$5.5 km footprint (Supplementary Methods).
...
To test whether the selection pattern generalizes beyond carbon, we ran a BCT-isomorphic pipeline on six NFTX vaults (Ethereum): all mint/redeem events and wallet-level vault-token transfers were reconstructed from on-chain logs, and each deposited or redeemed item was assigned the price percentile of its nearest sale within a rolling window of collection sales (percentiles control for market-level drift). Entry-margin selection was tested per vault (Wilcoxon, deposited-item percentiles against the collection median) and across vaults (sign test); exit-margin extraction by Mann--Whitney on redeemed versus deposited percentiles. Addresses handling more than 5\% of a side's events were flagged as routers and excluded from account-overlap statistics. A Curve stETH/ETH flow analysis is retained as supporting evidence (observational; also consistent with a classical bank run; Supplementary Methods).
