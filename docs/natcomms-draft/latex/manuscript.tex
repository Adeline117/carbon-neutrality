...
\textbf{Generalization across pools.} The pattern is not specific to one pool. Within the same operator, chain, period, and depositor population, the \textit{unscreened} pool's deposit quality degrades strongly over its life (Spearman $\rho$ = $-$0.439, $p < 10^{-40}$) while the \textit{screened} pool's does not ($\rho$ = $-$0.105, n.s.). The same degradation appears in a \textit{second, independent operator}: C3, a different bridge protocol, encodes each credit's Verra project and vintage in its token symbol, which we recovered on-chain (75 of 92 deposits scored, after typing projects via the public registry). C3's deposit quality also degrades ($\rho$ = $-$0.329, $p$ = 0.004), sits in the same low band as BCT (mean composite 31.1), and --- scored by type alone --- this drift is independent of vintage, a cleaner signal than BCT's largely vintage-driven decline. Moss MCO2, a single fungible basket token with no per-token credit identity, has no meaningful per-token quality, so adverse selection is structurally inapplicable there.
