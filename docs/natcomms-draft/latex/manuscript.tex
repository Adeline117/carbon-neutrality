...
\title[Article]{Transparency does not prevent quality collapse in carbon credit markets}
...
\begin{abstract}
Policymakers assume that making carbon credit quality visible will prevent low-quality credits from dominating trade. We show this assumption is wrong. We reconstruct every transaction in the largest pooled carbon credit market (22 million tonnes) and find that it collapsed despite full transparency. The cause was not hidden information but design: the pool sold every credit at one price regardless of quality, so participants could not act on what they saw. Informed buyers exploited this flaw, extracting \$4--\$8 million in high-value credits while 9.6 million tonnes of low-quality credits remain permanently stranded. Contrary to prevailing narratives, the collapse was driven by wind and solar credits with near-zero climate benefit (69.1\% of content, more than sixteen times the 4.2\% share attributed to forest conservation credits). Comparing identical credits across two pools with different rules, we find the same credits were fully extracted from the unscreened pool but only 28.5\% extracted from the quality-screened one. Market design, not credit type, determines asset fate. This transparency paradox carries direct implications for carbon trading systems now being designed under the Paris Agreement.
\end{abstract}
...
Blockchain-based tokenization promised to solve this problem through radical transparency. By recording every transaction on a public ledger, tokenized carbon pools would let any participant verify credit quality before trading. The first and largest such experiment was Toucan Protocol's Base Carbon Tonne (BCT) pool: a smart contract (an automated program on a blockchain, a tamper-proof digital ledger shared across thousands of computers) that accepted verified carbon credits from the Verra registry (the world's largest carbon credit standard) and issued fungible tokens (interchangeable units, each representing one tonne) at a single price, regardless of quality. BCT's price collapsed from approximately \$7 per token in late 2021 to below \$0.50 by mid-2023.
...
Here, we analyze every deposit (1,187), every withdrawal (35,432), every credit token (161), and every market participant (509 depositors, 28,897 redeemers) across BCT's entire operating history. The data contradict the prevailing narrative. BCT contained 69.1\% renewable energy credits --- predominantly wind farms and solar installations with near-zero additionality (meaning the projects would have been built with or without carbon revenue) --- and just 4.2\% REDD+. Five accounts extracted 1.55 million tonnes of high-demand credits at an estimated profit of \$4--\$8 million. Meanwhile, 9.6 million tonnes of low-quality renewables remain permanently stranded.

A natural experiment comparing identical credits across two pools with different quality screens sharpens the finding. The same credits were 100\% extracted from BCT but only 28.5\% extracted from a quality-gated pool. Pool design, not credit type, determines asset fate. Unlike prior work that quantifies overcrediting at the project level\textsuperscript{4,5}, we trace how overcredited assets produce participant-level profits and systematic quality collapse once they enter a pooled market --- even when quality differences are publicly observable.
...
We extracted all 1,187 deposit records from BCT's public transaction ledger, covering 168 unique Verra VCS projects and approximately 22 million tonnes of tokenized carbon credits (see Methods).
...
An illustrative satellite-based cross-check of 28 Indian CDM renewable energy projects in the BCT pool supports this assessment. Comparing country-level CDM-claimed emission factors (the amount of CO$_2$ assumed to be displaced per unit of electricity generated) against Ember's observed grid emission factors for the same vintage years reveals a mean overclaim of 26.5\% (range 23.6--28.8\%). Aggregate claimed emissions total 3.39 million tCO$_2$ versus 2.70 million tCO$_2$ consistent with observed grid data (see Supplementary Methods). An illustrative Sentinel-5P TROPOMI (a satellite instrument measuring atmospheric methane from space) methane column analysis of 9 BCT waste/methane projects found 5 of 9 (56\%) with site-level methane concentrations not significantly below background, though TROPOMI spatial resolution (5.5 km) limits project-level attribution.
...
\textbf{Bridge-level decomposition.} Enumerating all TCO2 tokens created by Toucan's factory contract reveals 369 unique tokens bridged before 2023, of which 345 (93.5\%) entered BCT. By tonnage, BCT absorbed 21.98 million of 22.07 million bridged tonnes (99.6\%). The 24 non-BCT tokens held a combined 86,683 tonnes (0.4\%). This near-complete pass-through shows that the 1.87$\times$ over-selection was a bridge-level phenomenon: Toucan's permissionless bridge disproportionately attracted renewable energy credits from Verra, rather than depositors selectively choosing renewables from a diverse bridged universe. Pool-level selection space was essentially zero.
...
\textbf{Granger causality} (a test of whether past values of one variable help predict another). At weekly frequency ($n$ = 55), we find asymmetric bidirectional Granger causality (Fig. 2). The dominant channel runs from price to quality: price changes precede quality changes ($F$ = 16.08, $p < 10^{-5}$ at lag 1--2), consistent with lower prices attracting lower-quality deposits. The reverse channel (quality preceding price) is significant but 2.5$\times$ weaker ($F$ = 6.32, $p$ = 0.004). This asymmetry fits the design-enabled framing: the architecture established the initial quality composition, which set a price level that then became the dominant driver of further quality deterioration. The architecture initiated the collapse; price discovery amplified it. The small sample ($n$ = 55 weeks) limits statistical power for Granger tests. The first-differenced daily regression ($n$ = 330, $\beta$ = $-$1.8, $p < 0.001$) provides more powerful confirmation that quality and price co-move at the expected sign. Rolling 30-day quality metrics show no Granger causality in either direction, confirming that the feedback operates through accumulated pool composition rather than marginal deposit quality. Given small-sample limitations, we present the Granger analysis as exploratory; our primary identification relies on the within-token cross-pool comparison and the first-differenced daily regression.
...
\begin{itemize}
\item \textbf{Cross-pool arbitrage (39.2\%)}: deposited into Toucan's NCT (Nature Carbon Tonne) pool, a quality-screened counterpart that shares identical protocol infrastructure (making it the closest available control), exploiting the price differential between BCT's uniform pricing and NCT's nature-based premium.
\item \textbf{Immediate retirement (34.6\%)}: permanently retired (formally cancelled, never reusable as emission offsets) within seconds of extraction, indicating pre-arranged retirement pipelines. The largest extractor (0x65a5..., 651,334 tonnes of industrial gas) retired 100\% of its credits in the same transaction as the redemption.
\item \textbf{Secondary market (17.0\%)}: transferred to other addresses, with 6.9\% re-deposited into BCT and 2.3\% still held.
\end{itemize}

The BCT-to-NCT arbitrage was systematic, not opportunistic. Of 31 tokens deposited into both BCT and NCT, 14 were redeemed from BCT, and all 14 satisfy the temporal sequence: BCT deposit $\rightarrow$ BCT redemption $\rightarrow$ NCT deposit (median: 103 days in BCT, then 14 days to NCT). No token violated this ordering.
...
\textbf{Exogenous market shock.} The May 2022 cryptocurrency market crash (see Methods) provides deposit-side evidence consistent with a causal channel. Pre-shock deposits ($n$ = 491, mean quality 32.9) versus post-shock deposits ($n$ = 87, mean quality 29.5) show a 3.4-point quality drop (Cohen's $d$ = 0.49). Volume simultaneously collapsed by 98\% (14.9M to 281k tonnes). The temporal quality degradation tripled in slope ($\rho$ = $-$0.12 pre-shock versus $\rho$ = $-$0.36 post-shock). The exogenous price shock accelerated quality deterioration and drove volume toward zero, consistent with the price-to-quality feedback channel identified above.

\textbf{Compositional shift.} Renewable share rose from 67\% (Q1) to 99.5\% (Q4). Token diversity collapsed from 24 distinct scores to 2. Higher-quality types (IFM, Waste/Methane, ARR) ceased depositing earlier (median blocks 20.4M--21.7M) than renewables (32.6M).

\textbf{Depositor concentration.} Deposits were highly concentrated: 509 accounts, with the top 10 accounting for 50\% of tonnage (Gini = 0.94). Large and small depositors deposited similar-quality credits (rank-biserial $r$ = $-$0.17; the volume-weighted gap of 5 quality points does not survive permutation, $p$ = 0.082).

\subsection*{Pool design determines asset fate: a within-token natural experiment}

The preceding results are descriptive: they establish \textit{what} the pool contained and \textit{who} extracted value, but not that pool architecture \textit{causes} the divergent fate of otherwise-identical credits. A clean test is available because 14 credit tokens (1.50 million tonnes) were deposited into \textit{both} the unscreened pool (BCT) and a quality-screened pool (NCT). The same physical credit therefore experienced two pool designs simultaneously, holding quality, vintage, registry, and project type fixed by construction. Each credit serves as its own control: a within-token matched-pair design in which the only variable that differs across the pair is pool architecture. Reconstructing per-token deposit and redemption tonnage directly from raw on-chain transfer logs reproduces the aggregate redemption rates exactly (100.0\% from the unscreened pool, 28.5\% from the screened pool).

Across the 14 matched pairs, the mean within-token redemption gap is +73.9 percentage points (unscreened minus screened) (Fig.~\ref{fig:within_token}). After dropping one credit whose redemption difference is a rounding artefact ($\approx$1 tonne of dust in 1.5 million), all 13 remaining pairs favour the unscreened pool. An exact paired sign test, an exact permutation test (full enumeration of per-token sign flips), and a Wilcoxon signed-rank test all return $p$ = $2.4 \times 10^{-4}$. A Bayesian Beta--Binomial paired model (deposited tonnes as trials, redeemed tonnes as successes, weakly informative priors) places the cross-token mean gap at +73.8 percentage points (95\% credible interval [72.2, 75.3]), with posterior probability 1.00 that the gap exceeds zero. The effect is not an artefact of credit-type composition: it is positive within \textit{every} type (improved forest management +89.0, afforestation/reforestation +100.0, REDD+ +38.0 percentage points). Because quality is held fixed within each pair, an unobserved confounder would have to raise the unscreened-versus-screened redemption odds by a factor of $\Gamma$ > 3.05 --- making a credit more than threefold more likely to be redeemed from one pool than the other for reasons unrelated to pool design --- before the result loses significance (Rosenbaum bound; Supplementary Methods). We report the design honestly as small ($n$ = 14): inference therefore rests on exact and Bayesian methods rather than asymptotics, and the credits that reached both pools may be non-representative, a selection channel we bracket with the sensitivity bound above and the directed acyclic graph in Supplementary Fig.~3.

\textbf{Generalization across pools.} The mechanism is not specific to a single pool. Within the same operator, chain, time window, and depositor population, the \textit{unscreened} pool's deposit quality degrades strongly over its operating life (Spearman $\rho$ = $-$0.439, $p < 10^{-40}$) while the \textit{screened} pool's does not ($\rho$ = $-$0.105, $p$ = 0.088, n.s.) --- a permissionless-versus-filtered contrast that isolates screening from operator, technology, and macro conditions. Two further bridge instruments (C3 and Moss MCO2) score in the same low band as the unscreened pool by methodology archetype (mean composite 34.0 and 26.7, both below the A threshold), but their per-token methodology and vintage are not recoverable from on-chain data, so a quality-over-time signal is not identifiable for them and we make no temporal claim; Moss MCO2 is in any case a fixed-supply bridge token rather than a deposit pool, so adverse selection is structurally absent. We document six such data gaps explicitly (Supplementary Methods) rather than impute unobserved quality.

\textbf{Pool-level difference-in-differences (descriptive).} A deposit-level DiD pooling all 1,895 events (1,187 BCT + 708 NCT), with $\text{PostShock}_i$ indicating deposits after the May 2022 market crash and standard errors clustered at the TCO2 token level, corroborates the divergence descriptively: the quality-score specification finds BCT quality falling 7.3 points more than NCT after the shock ($\hat{\beta}_3$ = $-$7.33, naive permutation $p < 0.0001$). We report this as descriptive only, not as a causal estimate, because BCT and NCT share 88.6\% of their tokens, violating the unit-independence its inference assumes. Under a token-clustered bootstrap the standard error inflates 4.02-fold and the between-pool rank difference is indistinguishable from zero (cluster-robust $p$ = 0.24). Causal identification therefore rests on the within-token design above.
...
The paper's core findings --- composition, base-rate over-selection, redemption-side extraction, within-token cross-pool comparison, and compositional DiD --- all rest on transaction data and Verra credit-type classifications. None depend on the quality scoring framework.
...
To assess whether the mechanism generalises beyond a single pool, deposits to two further bridge instruments (C3 and Moss MCO2) were scored by the same methodology-archetype procedure used for BCT. For BCT and NCT we report the temporal-quality slope (Spearman $\rho$ of composite vs deposit order) from the within-operator permissionless-versus-filtered contrast ($\rho$ = $-$0.439 vs $\rho$ = $-$0.105). For C3 and MCO2, per-token methodology and vintage are not recoverable from on-chain data, so a quality-over-time signal is not identifiable; we report their archetype-based composite level only and document each unrecoverable quantity explicitly (six data gaps) rather than impute it. Moss MCO2 is a fixed-supply bridge token rather than a deposit pool, so adverse selection is structurally inapplicable.
...
\caption{Within-token natural experiment: pool design determines the fate of identical credits. (\textbf{a}) Per-token slope plot of redemption (retirement) rate for the 14 carbon credit tokens deposited into \textit{both} the unscreened BCT pool and the quality-screened NCT pool, coloured by project type; the bold black line traces the pool means (100.0\% redeemed from the unscreened pool versus 28.5\% from the screened pool). Mean within-token gap +73.9 percentage points; exact sign, permutation, and Wilcoxon $p$ = $2.4 \times 10^{-4}$; Bayesian gap +73.8 pp (95\% credible interval [72.2, 75.3]). (\textbf{b}) Redemption rate by project type (tonnage-weighted): the gap is positive within every type (IFM +89, ARR +100, REDD+ +38 pp). The result is robust to unobserved confounding up to a Rosenbaum bound of $\Gamma$ > 3.05. Depositor-level temporal, base-rate, and difference-in-differences robustness panels are reported in Supplementary Fig.~3.}
