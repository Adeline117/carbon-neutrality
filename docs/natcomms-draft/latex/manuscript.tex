...
\bottomrule
\end{tabular}
These five accounts removed 1.55 million tonnes: the vast majority of redeemed industrial gas, 37\% of REDD+, and 44\% of ARR. The largest by tonnage (0x65a5\ldots; 651,334 tonnes of industrial gas, 42\% of the 1.55 million) is not an arbitrageur but a smart contract (verified on-chain; see Data availability) that retired every credit in the same transaction as redemption, the signature of a retirement aggregator routing credits to final use. The other four are individual wallets whose redeem-then-hold behaviour is consistent with strategic arbitrage of the gap between BCT's uniform price and higher off-chain values, roughly \$10 million in captured differential at midpoint price assumptions (bounds \$0.3--\$22M; an upper bound including retirement-routed volume; Methods). Fifteen of the twenty largest redeemers never deposited anything, and each targeted a single credit type.
...
The underlying logic (Gresham's Law under uniform pricing) applies wherever heterogeneous assets are pooled\textsuperscript{3,14}: six uniform-priced NFT vaults show the same entry-margin lemons selection, strongly in five (deposited-item median value percentiles 0.25--0.40 under strict sale matching, each $p < 10^{-10}$; marginal in the sixth) and with no exit-margin extraction in any; BCT, whose bridge passed heterogeneous credits in wholesale, expressed selection at both margins (Supplementary Methods).
