...
**Difference-in-differences: BCT declines faster than NCT.** A deposit-level difference-in-differences (DiD) panel pooling all 1,895 deposit events (1,187 BCT + 708 NCT) tests whether BCT's quality trajectory diverged from NCT's. Using the Terra/LUNA collapse as the treatment boundary, the event-study DiD yields a significant interaction: BCT quality fell 7.3 points more than NCT after Terra ($\hat{\beta}_3$ = $-$7.33, cluster-robust SE = 2.50, $t$ = $-$2.93, $p$ = 0.004). A permutation test shuffling pool labels across 10,000 iterations confirms this ($p$ < 0.0001); a Bayesian bootstrap places 99.98% posterior probability on BCT declining faster. Descriptively, BCT's mean quality dropped from 32.1 (pre-Terra) to 29.0 (post-Terra) while NCT's rose from 39.8 to 44.1. The divergence is consistent with the price-to-quality feedback channel: the Terra shock depressed BCT's price, attracting lower-quality deposits, while NCT's quality gate filtered them out.
