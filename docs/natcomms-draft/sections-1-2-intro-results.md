...
Here we analyse every deposit, every redemption, and every wallet address across BCT's entire operating history: 1,187 deposits, 35,432 redemptions, 161 unique credit tokens, 509 depositor wallets, 28,897 redeemer wallets.
...
The VCM exhibits a 10-fold quality range (Fig. 4): PQD from 0.076 (DACCS) to 0.759 (grid-connected renewables). BCT's PQD of 0.679 sits at the bottom. A within-pool permutation test ($z$ = $-$0.64, $p$ = 0.27) finds no evidence that depositors selected the worst tokens *within* the eligible universe --- the universe itself was uniformly low-quality (mean 32.3), so no within-pool selection was needed. A vintage gradient (pre-2020 PQD 0.687, 2024+ PQD 0.273) confirms the quality problem is concentrated in legacy credits. A BBB quality gate would have reduced BCT's PQD from 0.679 to 0.405; the Toucan CHAR pool (biochar allowlist, PQD 0.221) demonstrates that category restriction prevents quality collapse^9^.
