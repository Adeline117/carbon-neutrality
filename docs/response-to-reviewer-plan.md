...
- **锚点**: `manuscript.tex:154`(局限段, "descriptive, not causal identifications ... rest on only two pools ... dominant renewables are structurally excluded");`manuscript.tex:179`(Methods, "The estimand is the pool-design effect on dual-eligible credits ... a higher-quality nature-based subset rather than the full unscreened pool");Fig. 4 caption `manuscript.tex:285`("descriptive; a two-pool comparison, not a causal identification")。
...
- **Response (EN)**: Conceded in the text at line 113. Fallback: the mechanism claim survives entity overlap because uniform pricing, not population identity, connects the two margins; we can weaken "two distinct account populations" to a purely account-level statement if required.
...
- **锚点**: 标题 `manuscript.tex:18`("Transparency without pricing: a credit-level forensic account");摘要 `manuscript.tex:32`(最强句为 "We show this collapse also occurs when quality is fully and publicly observable");导言 `manuscript.tex:46`("strongly consistent with pool design")。
...
- **锚点**: `manuscript.tex:207`(Methods 定义, "the difference in expected climate value between the counterfactual pool and BCT's actual composition");`manuscript.tex:146`(两个 frame 并排出现的讨论段)。
- **已执行(2026-07-03)**: 过渡句已加入 146 行两个 frame 之间(中英稿同步)。
- **Response (EN)**: The metric is a compositional counterfactual (Methods, line 207), not a deceptive-retirement measure; the two frames at line 146 measure different objects and can both hold. Optional one-sentence clarifier bridging them; no substantive change.
...
- **锚点**: `manuscript.tex:79`(首选规格为一阶差分 OLS, "the credible specification for non-stationary series");`manuscript.tex:81`(Granger 仅作 "consistent with" 表述并指向补充材料);`manuscript.tex:199`(Methods 自认 "the small sample limits statistical power",周频 n=55);`manuscript.tex:122`(2022 年 5 月加密崩盘作为外生冲击, d=0.49 中等效应)。
...
*版本 / Version: 2026-07-03。本预案对应 commit `c7909d6` 之后的稿面;所有行号以当日 `manuscript.tex` 为准。*
